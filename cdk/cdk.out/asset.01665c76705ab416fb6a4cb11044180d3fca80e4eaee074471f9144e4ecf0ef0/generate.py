...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- Bedrock 메시지 포맷 구성 (안정성 강화) ---
        messages = []
        if system_prompt:
            messages.append({"role": "system", "content": system_prompt})

        # 기존 대화 기록 이어붙이기 (빈 메시지 제외)
        for m in chat_history:
            content = m.get("content", "").strip()
            if content:  # 내용이 있는 메시지만 추가
                messages.append({"role": m.get("role"), "content": content})
        
        # 현재 사용자 메시지 추가 (중복 역할 방지)
        if user_input:
            # 마지막 메시지가 user 역할이면 현재 입력을 거기에 합침
            if messages and messages[-1].get("role") == "user":
                messages[-1]["content"] += "\\n\\n" + user_input
            else:
                messages.append({"role": "user", "content": user_input})

        # Bedrock은 assistant로 시작하는 대화를 허용하지 않음
        # 시스템 프롬프트(있다면) 다음에 assistant 메시지가 오면 제거
        first_msg_idx = 1 if system_prompt else 0
        if len(messages) > first_msg_idx and messages[first_msg_idx].get("role") == "assistant":
            del messages[first_msg_idx]
