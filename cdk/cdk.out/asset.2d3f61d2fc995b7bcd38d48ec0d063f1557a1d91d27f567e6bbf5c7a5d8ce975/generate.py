...
def calculate_dynamic_max_tokens(user_input: str, system_prompt: str = "", model_id: str = DEFAULT_MODEL_ID) -> int:
    ...
    if input_length < 1000:
        ...
        dynamic_tokens = base_tokens * 5
    
    # 시스템 프롬프트 길이 고려
    if system_length > 5000:
        dynamic_tokens = max(dynamic_tokens, base_tokens * 2)
    ...
    try:
        ...
        if not project_id or not user_input:
            ...
            return {
                'statusCode': 400, 
                'headers': get_cors_headers(), 
                'body': json.dumps({'message': '프로젝트 ID와 사용자 입력은 필수입니다.'})
            }
        ...
        if not supports_ondemand:
            ...
            return {
                ...
                'headers': get_cors_headers(),
                'body': json.dumps({
                    'message': f'현재 모델({model_info["name"]})은 온디맨드 호출을 지원하지 않습니다.',
        ...
        # 모든 메시지를 하나로 합치고, 역할이 같은 연속 메시지를 병합
        ...
        logger.info(f"Bedrock 호출 시작 - 프로젝트: {project_id}, 모델: {model_info['name']}, 최대토큰: {max_tokens}")
