...
class PromptIndexer:
    ...
    def index_prompt(self, prompt_card: Dict[str, Any], prompt_text: str) -> bool:
        ...
        try:
            ...
            # 2. OpenSearch에 벡터 저장
            if self.embedding_processor.opensearch_client:
                opensearch_doc = {
                    'promptId': prompt_card['promptId'],
                    'projectId': prompt_card['projectId'],
                ...
                try:
                    self.embedding_processor.opensearch_client.index(
                        index='prompt-vectors',
                        id=prompt_card['promptId'],
                        body=opensearch_doc
                    )
            ...
            embedding_metadata = self.embedding_processor.create_embedding_metadata(
                {**prompt_card, 'prompt_text': prompt_text}, embedding
            )
...
def create_prompt_card(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"DynamoDB 저장 완료: 프로젝트 {project_id}, 프롬프트 {prompt_id}, stepOrder {next_step_order}")
