...
# Claude 3.5 Sonnet으로 변경 - 더 강력하고 긴 컨텍스트 처리 가능
...
def calculate_dynamic_max_tokens(user_input: str, system_prompt: str = "") -> int:
    ...
    if input_length < 1000:
        ...
        dynamic_tokens = base_tokens * 2  # 8192
    elif input_length < 8000:
        dynamic_tokens = base_tokens * 3  # 12288
    elif input_length < 15000:
        dynamic_tokens = base_tokens * 4  # 16384
    else:
        dynamic_tokens = base_tokens * 5  # 20480
    
    # 시스템 프롬프트 길이 고려 - 더 관대하게
    if system_length > 5000:
        dynamic_tokens = max(dynamic_tokens, base_tokens * 2)  # 최소 8192 보장
    
    # Claude 3.5 Sonnet의 최대 출력 토큰 제한 (200K 컨텍스트)
    max_tokens = min(dynamic_tokens, 8192)  # 출력 토큰 최대값
