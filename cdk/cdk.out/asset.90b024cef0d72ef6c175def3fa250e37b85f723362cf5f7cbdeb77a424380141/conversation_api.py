...
def handler(event, context):
    ...
    try:
        ...
        try:
            ...
            print(f"Event structure: {json.dumps(event, indent=2, default=str)}")
            
            # 임시로 개발환경에서만 fallback 허용 (추후 제거 예정)
            # TODO: 프로덕션에서는 이 부분을 제거해야 함
            user_sub = "temp-user-for-debug"
            print(f"WARNING: Using temporary fallback user_sub: {user_sub}")
            
            # 실제 운영환경에서는 아래 코드를 사용
            # return {
            #     'statusCode': 401,
            #     'headers': {
