...
class SimplePromptManager:
    ...
    def load_project_prompts(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
                ...
                try:
                    ...
                    if content:
                        prompts.append({
                            ...
                            'content': content,
                            'stepOrder': meta.get('stepOrder', 0),
            ...
            # stepOrder로 정렬 (없으면 생성 시간순)
            prompts.sort(key=lambda x: (x.get('stepOrder', 999), x.get('createdAt', '')))
    ...
    def get_project_prompt_stats(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            step_orders = [p.get('stepOrder', 0) for p in prompts]
            has_ordering = any(step > 0 for step in step_orders)
            ...
            return {
                ...
                "has_step_ordering": has_ordering,
                "step_range": {
                    "min": min(step_orders) if step_orders else 0,
                    "max": max(step_orders) if step_orders else 0
                },
                "prompts": [
                    {
                        "id": p['promptId'],
    ...
    def validate_prompts(self, prompts: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # 단계 순서 검사
        step_orders = [p.get('stepOrder', 0) for p in prompts]
        if len(set(step_orders)) != len(step_orders) and any(step > 0 for step in step_orders):
            warnings.append("일부 프롬프트의 단계 순서가 중복됩니다.")
