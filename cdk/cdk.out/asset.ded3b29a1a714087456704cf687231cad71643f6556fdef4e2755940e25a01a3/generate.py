...
# 3. 대화 메모리 설정
# - k=10: 최근 10개의 대화(사용자 질문 5개, AI 답변 5개)를 기억합니다.
# - return_messages=True: LangChain이 처리할 수 있는 메시지 객체로 기록을 반환
memory = ConversationBufferWindowMemory(k=10, memory_key="history", return_messages=True)

# 4. LangChain Runnable Chain (LCEL) 구성
chain = (
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        # --- 대화 기록을 메모리에 로드 ---
        # 이전 대화를 메모리에 채워넣어 AI가 기억하게 함
        memory.clear() # 이전 기록 초기화
