...
try:
    ...
    logger.warning("OpenSearch 라이브러리 없음 - 기본 모드로 실행")
...
def generate_title_with_rag(project_id: str, article_text: str) -> Dict[str, Any]:
    """RAG 기반 단계별 제목 생성"""
    try:
        # 1. OpenSearch 클라이언트 초기화
        opensearch_client = init_opensearch_client()
        ...
        # 3. 단계별 관련 프롬프트 검색 및 실행
        step_results = {}
        used_prompts = 0
        
        for step in range(1, 7):  # 6단계 워크플로우
            relevant_prompts = search_prompts_by_step(
                opensearch_client, project_id, article_embedding, step
            )
            
            if relevant_prompts:
                step_result = execute_step_with_prompts(
                    step, relevant_prompts, article_text, step_results
                )
                step_results[f"step_{step}"] = step_result
                used_prompts += len(relevant_prompts)
        ...
        logger.error(f"RAG 제목 생성 실패: {str(e)}")
        return None
...
def init_opensearch_client():
    ...
    try:
        ...
        if not endpoint:
            logger.warning("OPENSEARCH_ENDPOINT 환경변수 없음")
            return None
        ...
        logger.error(f"OpenSearch 초기화 실패: {str(e)}")
        return None
...
def create_embedding(text: str) -> List[float]:
    ...
    try:
        response = bedrock_runtime.invoke_model(
            ...
            body=json.dumps({"inputText": text[:8000]})  # Titan 제한
        )
...
def search_prompts_by_step(opensearch_client, project_id: str, article_embedding: List[float], step_order: int) -> List[Dict]:
    """단계별 관련 프롬프트 검색"""
    try:
        search_body = {
            "size": 3,
        ...
        response = opensearch_client.search(
            index='prompt-vectors',
            body=search_body
        )
        
        prompts = []
        for hit in response['hits']['hits']:
            if hit['_score'] > 0.5:  # 유사도 임계값
                prompt_data = hit['_source']
                prompt_text = load_prompt_from_s3_fast(prompt_data.get('s3Key', ''))
        ...
        logger.error(f"프롬프트 검색 실패: {str(e)}")
        return []
