...
def save_to_opensearch(parsed_info: Dict[str, str], text: str, embedding: List[float]) -> None:
    ...
    try:
        ...
        # OpenSearch 엔드포인트 URL 구성
        url = f"https://{OPENSEARCH_ENDPOINT}/{INDEX_NAME}/_doc/{parsed_info['fullPath']}"
        ...
        SigV4Auth(credentials, 'es', os.environ['REGION']).add_auth(aws_request)
        ...
        if response.status_code not in [200, 201]:
            raise Exception(f"OpenSearch 저장 실패: {response.status_code} - {response.text}")
        
        logger.info(f"OpenSearch 저장 성공: {parsed_info['fullPath']}")
