...
def list_categories(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 카테고리만 조회 (projectId가 category#으로 시작하는 것들)
        response = table.scan(
            FilterExpression='begins_with(#projectId, :category_prefix) AND #ownerId = :ownerId',
        ...
        for item in response.get('Items', []):
            # category# 접두사 제거하여 실제 카테고리 ID 추출
        ...
        # 생성일시순 정렬
        categories.sort(key=lambda x: x.get('createdAt', ''), reverse=True)
...
def list_projects(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 스캔 파라미터 구성 - 카테고리가 아닌 실제 프로젝트만 조회 (active 상태만)
        scan_params = {
            'Limit': limit,
